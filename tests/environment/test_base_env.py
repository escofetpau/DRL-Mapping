...
def base_env(circuit_config):
    ...
    env.n_qubits = circuit_config["n_qubits"]

    return env


def test_get_all_lookaheads(base_env):
    base_env.n_slices = 3
    base_env.n_qubits = 3
    ...
    lookahead = base_env._get_all_lookaheads(circuit, sigma=1)

    assert lookahead.shape == (base_env.n_slices, base_env.n_qubits, base_env.n_qubits)
